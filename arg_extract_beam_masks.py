...
from torch import tensor, cat, float32
    ...
    for pose_idx in range(N_T8_POSES):
        ...
        with h5py.File(ppdf_path, "r") as f:
            ...
            pose_row_sampling_info = read_row_sampling_metadata(
                f,
                ppdf_shape_first_dim=ppdfs_data.shape[0],
            )
...
def run_one(
    ...
    *,
    ...
    if ppdfs_override is None:
        ...
        with h5py.File(ppdf_path, "r") as f:
            row_sampling_info = read_row_sampling_metadata(
                f,
                ppdf_shape_first_dim=ppdfs.shape[0],
            )
        ...
        if sampled_detector_unit_indices is None:
            raise ValueError("sampled_detector_unit_indices is required with ppdfs_override")
    ...
    total_rows = int(row_sampling_info.get("srm_total_rows", int(detector_units_vertices.shape[0])))
    extra_attrs = {
    ...
    n_detector_units = int(detector_units_vertices.shape[0])
    if ppdfs.shape[0] != n_sampled_rows:
        raise ValueError(
            f"PPDF row count {ppdfs.shape[0]} does not match sampled row count {n_sampled_rows}"
        )
    print(
